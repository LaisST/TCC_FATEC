...
<body class="bg-[url('../img/img-fundo.svg')] bg-no-repeat h-screen bg-top bg-indigo-100 bg-contain bg-opacity-80 flex items-center justify-center min-[450px]:py-4 sm:px-4">
    <section class="w-full h-full max-w-lg p-6 font-light bg-white min-[450px]:rounded-2xl drop-shadow-xl text-slate-800 sm:h-auto">
        ...
        <div class="bg-[url('../img/img-login-estudante.svg')] bg-no-repeat border-b bg-center border-solid border-slate-100 bg-contain h-52 rounded-t-2xl"></div>
        <div class="flex flex-col pt-8 font-sm gap-y-6">
            ...
            <form class="flex flex-col select-none gap-y-6" method='POST' action="../controller/AlunoController.php?acao=login">
                <div class="flex flex-col">
                    ...
                    <input class="bg-slate-50 focus:bg-slate-100 focus:border-[#3829e082] transition-colors duration-300 p-2 mt-1 border border-solid rounded-lg outline-none placeholder-slate-300 border-slate-200" type="number" name="ra" id="ra" placeholder="R.A" min="1234567891023" maxlength="13" oninput="if (this.value.length = this.maxLength) this.value = this.value.slice(0, this.maxLength)" autocomplete="off" required>
                </div>
                <div class="flex flex-col">
                    ...
                    <a class="mt-2 hover:text-[#3829e0] transition-colors hover:underline w-max text-slate-400" href="redefinir_senha.php">Esqueci minha senha</a>
                </div>
