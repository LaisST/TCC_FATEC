...
data_Intervalo = data_atual + datetime.timedelta(days=5)
...
consulta = """SELECT DISTINCT
            ...
            FROM
                aluno
            WHERE
                aluno.data_expiracao_senha > %s and aluno.data_expiracao_senha <= %s
            GROUP BY
                aluno.RA, aluno.email
            HAVING
                COUNT(aluno.data_expiracao_senha) > 0;"""
cursor.execute(consulta, (data_atual, data_Intervalo))
...
for usuario in usuarios:
    ...
    mensagem['Subject'] = 'Expiração de Senha'
    ...
    corpo_email = f"""
        ...
        <head>
            ...
            <title>Notificação de Expiração de Senha</title>
        </head>
        <body style="font-family: Arial, sans-serif;">

            <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
                
                <h2 style="color: #333;">Notificação de Expiração de Senha</h2>
                
                <p>Olá {usuario[2].split()[0]},</p>
                
                <p>Este é um lembrete de que sua senha está prestes a expirar. Por favor, tome as medidas necessárias para atualizar sua senha antes da data de expiração.</p>
