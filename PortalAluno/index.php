...
<body class="bg-[url('../img/img-fundo.svg')] bg-no-repeat h-screen bg-top bg-indigo-100 bg-contain bg-opacity-80 flex items-center justify-center min-[450px]:py-4 sm:px-4">
    <section class="min-[450px]:rounded-2xl drop-shadow-xl bg-white font-light max-w-md text-slate-800 h-full w-full sm:h-auto sm:w-[30rem] p-6">
        <div class="bg-[url('../img/img-tela-inicial.svg')] bg-no-repeat border-b bg-center border-solid border-slate-100 bg-contain h-52 rounded-t-2xl"></div>
        <div class="flex flex-col pt-8 font-sm gap-y-6">
            ...
            <div class="grid grid-cols-2 gap-x-4">
                ...
                <label class="flex flex-col items-center px-6 py-10 transition-opacity duration-300 border border-solid rounded-lg cursor-pointer select-none sm:text-lg gap-y-2 bg-slate-100 border-slate-200 opacity-40 label-identificador" for="estudante">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1" stroke="currentColor" class="w-8 h-8 transition-colors duration-300 sm:w-9 sm:h-9">
            ...
            <button class="rounded-xl sm:text-lg font-normal py-3.5 text-white bg-[#3829e0] hover:bg-[#2b3ff7] transition-colors duration-300 cursor-pointer" onclick="redirecionar()">Continuar</button>
        </div>
    </section>
</body>
<script>
    function redirecionar() {
        if (document.getElementById('estudante').checked) {
            ...
            window.location.href = 'src/view/login-professor.php';
        }
    }
</script>
</html>
